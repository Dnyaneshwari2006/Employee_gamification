...
public class CeoprojectdetailsActivity extends AppCompatActivity {
...
//                         Ensure list of employees exists
                        ...
                        Toast.makeText(CeoprojectdetailsActivity.this, "Project not found!", Toast.LENGTH_SHORT).show();
                        finish();
                    }
                });
    ...
    private void loadTasks(String projectId) {
        ...
        taskRef.get().addOnCompleteListener(taskTask -> {
            if (taskTask.isSuccessful() && taskTask.getResult() != null) {
                List<CEO_projectdetails> taskList = new ArrayList<>();
                ...
                for (QueryDocumentSnapshot doc : taskTask.getResult()) {
                    try {
                        ...
                        taskList.add(new CEO_projectdetails(assignedTo,id, taskDetail, deadline, status,name));
                ...
                // Set Adapter
                CeoProjectDetailsAdapter adapter = new CeoProjectDetailsAdapter(taskList);
                recyclerView.setAdapter(adapter);
                recyclerView.setLayoutManager(new LinearLayoutManager(CeoprojectdetailsActivity.this));
            } else {
